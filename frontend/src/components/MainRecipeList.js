        ...
        if (num == 0)
        {
            ...
            for(let i = 0; i < popularRef.current.length;i++)
            {
                if(cnt==3)
                    break;
                result.push(
                    <a style={{cursor:'pointer'}}><img style={{width:"300px", height:"300px"}}  src={"data:image/png;base64,"+popularRef.current[i].recipeImg} alt="1" onClick={(e) => window.location.href = "/zipcook/RecipeAttractionInfo?videoNumber=" + popularRef.current[i].videoNumber}/><p>{popularRef.current[i].videoTitle}</p></a>
                );
                cnt= cnt+1;
            }
        }
        else
        {
            for(let i = 0; i < preferRef.curren.length;i++)
            {
                if(cnt==3)
                    break;
                result.push(
                    <a style={{cursor:'pointer'}}><img  style={{width:"300px", height:"300px"}} src={"data:image/png;base64,"+preferRef.current[i].recipeImg} alt="1" onClick={(e) => window.location.href = "/zipcook/RecipeAttractionInfo?videoNumber=" + preferRef.current[i].videoNumber}/><p>{preferRef.current[i].videoTitle}</p></a>
                );
                cnt= cnt+1;
            }
        }
