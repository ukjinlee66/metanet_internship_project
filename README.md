...
- <h4>또한 통신매체의 발달로 소비자는 음식의 다양성을 접하고, 이를 접해보려는 시도가 증가하고 있다. </h4>
- <h4>영상을 통한 요리를 하는 사람들의 수가 점점 많아지고 있으며 각 분야 (한식,양식,중식,일식)와 난이도(초급,중급,상급)을 통해 자신이 좋아하는 분야의 난이도에맞는 검색 수단으로써 영상을 통해 쉽게 제공받을 수 있도록 기획하게 되었다.</h4>
...
- 자바 표준 ORM인 JPA를 사용하여 데이터 처리과정을 퀴리중심이 아닌 객체지향적으로 진행하였다. 
- FrontEnd 와 Backend가 분리된 환경에 Axios를 기반으로 통신하는 법을 배웠으며 Swagger 툴을 통해 분리된 개발환경에서 협업의 효율성을 높혔다.
- 미디어 타입 통신을 위한 인코딩, 디코딩 과정을 배웠고, HLS프로토콜을 통한 스트리밍의 작동원리를 배웠다.
- SW 개발 프로세스의 산출문서 작성과 DB 설계의 중요성에 대해 배웠다.
...
- SNS 로그인 API, Eleasticsearch API 등 다양한 API를 사용해보지 못한 점.
...
- HLS  - [https://www.cloudflare.com/ko-kr/learning/video/what-is-http-live-streaming/], [https://www.npmjs.com/package/react-hls-player]
